...
namespace DotNetMigrations.UnitTests.Commands
{
	...
	public class VersionCommandIntegrationTests : DatabaseIntegrationTests
	{
		...
		[Test]
		[Ignore("We are using functions that the test database(SqlCeEngine) doesn't support")]
		public void Run_should_create_schema_migrations_table_if_it_doesnt_exist()
		{
			//  arrange
			_mockMigrationDir.Setup(dir => dir.GetScripts(It.IsAny<string>())).Returns(Enumerable.Empty<IMigrationScriptFile>);
		...
		public void Run_should_log_latest_script_version()
		{
			...
			_mockMigrationDir.Setup(dir => dir.GetScripts(It.IsAny<string>())).Returns(scriptFiles);
