...
namespace DotNetMigrations.UnitTests.Data
{
	...
	public class DataAccessIntegrationTests : DatabaseIntegrationTests
	{
		...
		{
			CreateDatabase();
			_subject = DataAccessFactory.Create(TestConnectionString, new LogRepository());
		}
		...
		{
			// arrange
			_subject = DataAccessFactory.Create(TestConnectionString + ";CommandTimeout=1", new LogRepository());
